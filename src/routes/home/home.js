...
import AlertDialog from "../../components/alterDialog/alertDialog";
...
export function MainHome() {
  ...
  useEffect(() => {
    const lo = localStorage.getItem("links");
    if (lo) {
      setUserLinks(JSON.parse(lo));
    }
  }, [userLinks]);
    ...
    <div className="text-blue-600">
      <div>
        {userLinks.length != 0 ? (
            ...
            return (
              <div>
                <div class="hover:bg-gray-200 mt-[1px] mb-[1px] flex items-start  border-blue-400 bg-white p-4 shadow-lg border-l-0 border-r-0 border-t-0">
                  <div
                    className="flex justify-center content-center mt-auto mb-auto"
                  ...
                  >
                    {1 ? (
                      ...
                      <img src={e.img} className="w-[28px] h-[28px]" />
                    )}
                  </div>
                  <div class="ml-4  w-full overflow-hidden text-left p-2 hover:cursor-pointer"
                    onClick={() => {
                      window.chrome.tabs.create({ url: e.url, active: false });
                    }}
                  >
                    <h2 class="font-semibold truncate ">{e.title}</h2>
                    <p class="mt-2 text-sm text-gray-500 truncate hover:cursor-pointer">
                      {e.url}
                      {/* <img src={currentUrl} className="w-[32px] h-[32px]"/> */}
                    </p>
                  </div>
                  <div className="mt-auto mb-auto hover:cursor-crosshair">
                    <AlertDialog id={e.id} title={e.title} />
                  </div>
                </div>
              </div>
            );
          ...
          <div className="w-full mt-40">Add New Links</div>
        )}
      </div>
    </div>
  );
